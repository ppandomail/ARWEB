...
import ar.edu.uade.tic.tesis.arweb.controlador.ControlInformacion;
import ar.edu.uade.tic.tesis.arweb.controlador.GestorBaseDatos;
import ar.edu.uade.tic.tesis.arweb.controlador.GestorContadorAccesibilidad;
...
public class Resultados extends JFrame implements ActionListener, MouseListener, HyperlinkListener {
	...
	private JPanel getPanelEncabezado(String principios) {
		...
		jPanelEncabezado.add(this.getPanelEncabezadoHistorico());
	...
	private JPanel getPanelEncabezadoInformacion(String principios) {
		...
		jPanelEncabezadoInformacion.add(new JLabel("Cantidad Problemas : ", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_ERROR)), JLabel.LEFT));
		this.setjLabelCantidadProblemas(new JLabel(""));
		jPanelEncabezadoInformacion.add(this.getjLabelCantidadProblemas());
		jPanelEncabezadoInformacion.add(new JLabel("Cantidad Advertencias : ", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_MANUAL)), JLabel.LEFT));
		this.setjLabelCantidadAdvertencias(new JLabel(""));
		jPanelEncabezadoInformacion.add(this.getjLabelCantidadAdvertencias());
		jPanelEncabezadoInformacion.add(new JLabel("Cantidad No verificados : ", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_IMPOSIBLE)), JLabel.LEFT));
	...
	private JPanel getPanelEncabezadoHistorico() {
		JPanel jPanelEncabezadoHistorico = new JPanel();
		jPanelEncabezadoHistorico.setBackground(Color.WHITE);
		jPanelEncabezadoHistorico.setBorder(BorderFactory.createTitledBorder("<HTML><H3>Histórico de Verificación del Recurso</H3></HTML>"));
		jPanelEncabezadoHistorico.setLayout(new GridLayout(11, 2));
		ControlInformacion infoController = GestorBaseDatos.seleccionar(this.getTipoValidacion(), this.getRecurso());
		AnalisisDAO analisisDAO = null;
		if (infoController.isOk())
			analisisDAO = (AnalisisDAO) Contexto.get(ContextoClaves.ANALISIS_DAO);
		jPanelEncabezadoHistorico.add(new JLabel("Recurso : "));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(analisisDAO.getUrl()));
		jPanelEncabezadoHistorico.add(new JLabel("Fecha / Hora : "));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(analisisDAO.getFechaHora()));
		jPanelEncabezadoHistorico.add(new JLabel("Pautas Aplicadas : "));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel("WCAG 2.0"));
		jPanelEncabezadoHistorico.add(new JLabel("Principios : "));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(analisisDAO.getPrincipios()));
		jPanelEncabezadoHistorico.add(new JLabel("Nivel Accesibilidad : "));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(analisisDAO.getNivelAccesibilidad() + " " + "(umbral: " + analisisDAO.getUmbralString() + " puntos)"));
		jPanelEncabezadoHistorico.add(new JLabel("Tecnologías Verificadas : "));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel("HTML y CSS"));
		jPanelEncabezadoHistorico.add(new JLabel("Cantidad Problemas : ", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_ERROR)), JLabel.LEFT));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(String.valueOf(analisisDAO.getProblemasString())));
		jPanelEncabezadoHistorico.add(new JLabel("Cantidad Advertencias : ", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_MANUAL)), JLabel.LEFT));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(String.valueOf(analisisDAO.getAdvertenciasString())));
		jPanelEncabezadoHistorico.add(new JLabel("Cantidad No verificados : ", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_IMPOSIBLE)), JLabel.LEFT));
		jPanelEncabezadoHistorico.add(analisisDAO == null ? new JLabel("---") : new JLabel(String.valueOf(analisisDAO.getNoVerificadosString())));
		jPanelEncabezadoHistorico.add(new JLabel(""));
		jPanelEncabezadoHistorico.add(new JLabel(""));
		jPanelEncabezadoHistorico.add(new JLabel("Puntos:  ", JLabel.RIGHT));
		if (analisisDAO == null)
			jPanelEncabezadoHistorico.add(new JLabel("---", JLabel.LEFT));
		else {
			JLabel accesibilidad = new JLabel();
			accesibilidad.setHorizontalAlignment(JLabel.LEFT);
			if (analisisDAO.getAccesibilidad() >= analisisDAO.getUmbral()) {
				accesibilidad.setText(analisisDAO.getAccesibilidad() + "  -  " + Constantes.MENSAJE_ACCESIBLE);
				accesibilidad.setForeground(Color.GREEN.darker());
			} else {
				accesibilidad.setText(analisisDAO.getAccesibilidad() + "  -  " + Constantes.MENSAJE_NO_ACCESIBLE);
				accesibilidad.setForeground(Color.RED.darker());
			}
			jPanelEncabezadoHistorico.add(accesibilidad);
		}
		return jPanelEncabezadoHistorico;
	}
	...
	private JPanel getPanelBotonExportadores() {
		...
		this.setjButtonAnexoPDF(new JButton("Exportar Anexo PDF", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_LOGO_PDF))));
		...
		this.setjButtonDetallePDF(new JButton("Exportar Detalle PDF", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_LOGO_PDF))));
		...
		this.setjButtonDetalleXLS(new JButton("Exportar Detalle XLS", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_LOGO_XLS))));
		...
		this.setjButtonHTML(new JButton("Ver Código HTML", new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_LOGO_HTML))));
	...
	private JPanel getPanelDetalle(String principios, List<ResultadoEvaluacionPrincipio> resultadoValidacionGeneral) {
					...
					for (ResultadoEvaluacionTecnica resultadoEvaluacionTecnica : resultadoEvaluacionCriterio
						...
						for (ResultadoEvaluacionTecnicaItem resultadoEvaluacionTecnicaItem : resultadoEvaluacionTecnica.getResultadoEvaluacionTecnicaItems()) {
							...
							if (!marcaPrincipio.equals(resultadoEvaluacionPrincipio.getPrincipio().toString()))
								fila[0] = new Etiqueta(resultadoEvaluacionPrincipio.getPrincipio(), resultadoEvaluacionPrincipio.getPrincipio().toString(), new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_HELP)));
							if (!marcaPauta.equals(resultadoEvaluacionPauta.getPauta().toString()))
								fila[1] = new Etiqueta(resultadoEvaluacionPauta.getPauta(), resultadoEvaluacionPauta.getPauta().toString(), new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_HELP)));
							if (!marcaCriterio.equals(resultadoEvaluacionCriterio.getCriterio().toString())) {
								fila[2] = new Etiqueta(resultadoEvaluacionCriterio.getCriterio(), resultadoEvaluacionCriterio.getCriterio().toString(), new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_HELP)));
							...
							if (!marcaTecnica.equals(resultadoEvaluacionTecnica.getTecnica().toString())) 
								fila[4] = new Etiqueta(resultadoEvaluacionTecnica.getTecnica(), Utilidades.getTextoFormateadoConLongitud(resultadoEvaluacionTecnica.getTecnica().toString(), 6), new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_HELP)));
							fila[5] = resultadoEvaluacionTecnicaItem.getTipologia().getRepresentacionString();
							fila[6] = new Etiqueta(resultadoEvaluacionTecnicaItem, resultadoEvaluacionTecnicaItem.getNombre(), new ImageIcon(this.getClass().getResource(Constantes.PATH_IMAGE_HELP)));
							fila[7] = new JLabel(new ImageIcon(this.getClass().getResource(resultadoEvaluacionTecnicaItem.getTipoResultadoEvaluacion().getRepresentacionPathImagen())));
		...
		ControlInformacion infoController = GestorBaseDatos.insertar(this.getTipoValidacion(),
				this.getRecurso(), Utilidades.separarCaracteresPorComa(principios), "A",
				Constantes.PUNTAJE_UMBRAL,
				GestorContadorAccesibilidad.getCantidad(TipoResultadoEvaluacion.ERROR),
				GestorContadorAccesibilidad.getCantidad(TipoResultadoEvaluacion.MANUAL),
				GestorContadorAccesibilidad.getCantidad(TipoResultadoEvaluacion.IMPOSIBLE), this.getPuntosTotal() * 4);
		if (!infoController.isOk())
			JOptionPane.showMessageDialog(this, infoController.getMensaje(), Constantes.TITULO_PRINCIPAL, JOptionPane.ERROR_MESSAGE);
		return jPanelExportador;
	}
